...
- **cronjob**：`API 狀態監視器` — 每 4h 自動執行，從 `~/.hermes/env/agnes.env` 讀取金鑰
- **警報閾值**：配額 < 100 → 🔴 緊急通知；< 300 → 🟡 注意

## 金鑰管理

Agnes 金鑰單獨存放於 `~/.hermes/env/agnes.env`。模組化架構：

| 檔案 | 用途 |
|:-----|:-----|
| `~/.hermes/env/agnes.env` | 圖片/影片生成 (Agnes) |
| `~/.hermes/env/deepseek.env` | 文字/推理/程式碼 (DeepSeek) |
| `~/.hermes/env/freellmapi.env` | 本地測試 (FreeLLM) |
| `~/.hermes/env/telegram.env` | Telegram 機器人 |

`.bashrc` 自動 source 全部：`for f in $HOME/.hermes/env/*.env; do source "$f"; done`

## 參考文件

| 文件 | 內容 |
|:-----|:------|
| `references/agnes-image-2.1-flash.md` | 官方 Image API — 文生圖/圖生圖/參數/錯誤排查 |
| `references/agnes-video-v2.0.md` | 官方 Video API — 文生影/圖生影/多圖/關鍵幀/輪詢 |
