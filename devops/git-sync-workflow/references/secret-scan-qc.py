...
Usage:
  python3 secret-scan-qc.py [--repos repo1,repo2,...]
  python3 secret-scan-qc.py  # scans default repos
...
def classify_match(filepath, line, pattern_matched):
    """Classify as REAL => needs remediation, or FP => safe."""
    ...
    # process.env references are not leaks
    if 'process.env.' in line:
        return 'FP (env reference)'

    # d.get("password") / .get("password") = form parsing code
    if re.search(r'\.get\(\s*["\']password["\']\s*\)', line):
        return 'FP (form parsing)'

    # Markdown content in docs/ or references/
...
def scan_repo(name, path):
    """Scan a single repo. Returns (repo_name, has_real_leaks, results_list)."""
    git_dir = os.path.join(path, '.git')
    if not os.path.isdir(git_dir):
        return (name, False, [f"  ⏭️  Not a git repo"])
...
def main():
    ...
    for name, path in repos:
        print(f"\n📁 {name}")
        repo_name, has_real, results = scan_repo(name, path)
        if has_real:
            any_real_leak = True
            all_clean = False
    ...
    print("\n" + "=" * 60)
    if any_real_leak:
        print("❌ REAL LEAKS DETECTED — do not push until remediated")
        sys.exit(1)
    elif all_clean:
        print("✅ All repos clean — safe to push")
        sys.exit(0)
    else:
        print("⚠️  False positives only — safe to push")
        sys.exit(0)
