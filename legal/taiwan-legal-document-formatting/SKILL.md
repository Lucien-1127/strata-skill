...
**⚠️ 路徑陷阱（2026-07-08）**：LibreOffice 的 `--convert-to` 不一定將 PDF 產出到 .docx 所在目錄。若未指定 `--outdir`，PDF 寫入**目前工作目錄（cwd）**。

**修正：**
```bash
# 指定輸出目錄（最穩）
libreoffice --headless --convert-to pdf input.docx --outdir $(dirname input.docx)

# 或轉換後從 cwd 手動搬
cp ./input.pdf /target/dir/
```
...
### 重要：合約內容復刻規則（2026-07-08 新增）

當使用者提供圖片/影本作為合約範本要求重製時，**必須遵守以下規則**，違反將直接導致重新製作：

1. **保留範本原貌** — 不得自行增刪條款、加入額外說明、填寫空白欄位，或更改原文用字。使用者的範本內容是唯一基準。
2. **只處理排版與錯字** — 允許的變動僅限：
   - 設定正確字型（抵押權設定契約用標楷體 cwTeXKai）
   - 調整邊距、行距、字級
   - 修正明顯的 OCR 錯別字
   - 條號加粗體、項次縮排
   - 簽名欄改為直式（甲方/乙方各自獨立區塊）
   - 加入不動產標示表格
3. **禁止新增內容** — 不得加入「利率上限告知」「免責聲明」「注意事項」等範本未有的區塊。即使內容合法（如民法第205條），只要範本沒有就不得加入。
4. **禁止自行填寫利率** — 範本中「月息______%」的空白欄位，不得填入2%或其他數值。留空由使用者填寫。即使使用者口頭說過利率，也不應寫入合約。
5. **禁止加入範本未有的違約金計算公式** — 如「每萬元每日______元」「千分之一」等用語，不在範本中就不可以加。
6. **簽名欄格式** — 使用直式（分列甲方/乙方區塊，姓名、身分證、住址、電話直列），**不可使用表格**。
7. **確認簽約人名稱與範本一致** — 如範本使用「義務人即債務人（甲方）」，則不得改為「甲方（貸與人）」。
...
4. **特殊情況 — 抵押權設定契約書**（需送地政機關登記）：雖然是民間合約，但**字型應使用標楷體（cwTeXKai）而非 Noto Sans CJK TC**。地政機關審查慣例要求標楷體。同時邊距應加寬裝訂側至 2.8cm 以供騎縫章。詳細規格見 `references/contract-template-guide.md`。
5. 若文件含**長表格** → 額外套用「表格跨頁」規則
6. 若最終輸出為 **PDF** → 額外套用「PDF 重檔技巧」章節
...
- `references/contract-template-guide.md` — 民間借款抵押權設定契約書生成參考（11條結構、排版規格、費用標準、簽名欄表格規格、第一頁布局規則）
- `references/mortgage-contract-research-20260708.md` — 2026-07-08 抵押權設定契約書深度研究摘要（法條對照表、關鍵排版禁止事項、來源可達性）
- `references/預告登記關鍵判決.md` — 5 則最高法院/高等法院預告登記判決摘要
- `references/mortgage-fee-standards.md` — 抵押權設定登記法定規費標準對照表（含土地法§76、印花稅法§7④、地政規費） — 5 則最高法院/高等法院預告登記判決摘要
...
- ❌ **2026-07-08：抵押權設定契約書誤用 Noto 字型** — 需送地政機關登記的抵押權設定契約書，字型**必須使用標楷體（cwTeXKai）**，不可使用 Noto Sans CJK TC 或其他非楷書字型。地政事務所審查時可能因此要求修改。
- ❌ **2026-07-08：抵押權設定契約書第一頁甲方/乙方資訊混排** — 甲方（貸與人）與乙方（借用人）的基本資料應獨立為「立契約書人」段落後分列各行，不可放在標題副標同一行或與標題區塊混排。
- ❌ **2026-07-08：契約簽名欄使用文字底線填空** — 地政機關不接受文字底線格式的簽名欄。必須使用 Table Grid 雙欄表格，並將最後一列合併為日期欄（中　華　民　國　○年　○月　○日）。
- ❌ **2026-07-08：法律文件注意事項使用 emoji** — 法律文件的免責/注意事項區塊**禁止使用 emoji 符號**（如⚠️）。應使用標準文字「注意事項」作為標題。
- ❌ **2026-07-08：使用手機拍攝螢幕之模糊照片進行 OCR 導致條文錯誤** — 使用者提供 8 張 Adobe Acrobat 畫面照片，OCR 品質極差，導致「每萬元每日」「千分之一」等範本未有用語被誤讀為條文。**正確做法**：當使用者提供的來源是模糊照片/影本時，應先說明 OCR 限制，要求使用者提供 PDF 原檔或逐條確認，不可自行猜測條文內容填入。參考 `references/contract-template-20260708.md`。
- ❌ **2026-07-08：契約利率預設逾民法§205上限** — 民間借貸契約範本應預設年息 16%（非舊慣例的月息 2%=年息 24%），並在注意事項中明確載明「超過年息 16% 部分無請求權」。
