...
## 🚀 PDF 產出 QA 管線（2026-07-07 二模型審查制）

所有文件統一走：**python-docx → LibreOffice → PDF → 🤖子代理A排版檢查 → 🤖子代理B引用檢查 → 修正 → 交付**

### 雙子代理平行 QA 流程

PDF 產出後，**必須派遣兩個平行子代理**進行品質檢查：

```python
delegate_task(tasks=[
    {
        "goal": "對以下 PDF 進行排版與格式審查，列出所有不符合規範之處",
        "context": f"""
PDF 路徑：{pdf_path}

審查標準（taiwan-legal-document-formatting 技能）：
1. 軌道選擇是否正確？（法院→cwTeXKai 14pt / 政府→AR PL UKai TW 12pt / 合約→Noto Sans CJK TC 12pt）
2. A4 尺寸（595 x 842 pts），邊距 2.5cm
3. 字型是否正確嵌入？（pdffonts 檢查 cwTeXKai/AR PL 是否顯示 yes yes yes）
4. 有無不當頁面切分？（標題獨自在一頁底部、判決內容從中間被切開到下一頁）
5. 孤行控制是否生效？（頁首或頁尾無單獨一行）
6. 表格跨頁時標題列是否有重複？
7. 頁碼格式是否正確？
8. 整體視覺排版是否專業？

請逐項回報 ✅ 通過或 ❌ 不合格，並對不合格項目給出具體修正建議。
"""
    },
    {
        "goal": "對以下法律報告進行引用完整性審查，列出所有引用錯誤",
        "context": f"""
PDF 路徑：{pdf_path}

審查標準：
1. 判決引用：每個字號是否完整？（格式：最高法院OO年度台上字第XXXX號）
2. 學說引用：是否掛正確來源標籤？（^[學者名]，不是^[全國法規資料庫]）
3. 法條引用：條號是否正確？與全國法規資料庫最新版是否一致？
4. 函釋引用：函號是否完整？（含發文機關、日期、字號）
5. G3 格式是否統一？（^[來源簡稱] 格式）
6. 有無未附來源的主張？
7. 有無「相關判決」這種無字號的模糊用語？

請逐項回報 ✅ 通過或 ❌ 有問題，並對有問題項目給出具體修正建議。
"""
    }
])
```
...
1. 若兩個子代理都回報 ✅ **全部通過** → 直接交付
2. 若有任何 ❌ → **必須修正後重新生成 PDF，再跑一次 QA**，直到全部通過
3. 同一錯誤連續出現兩次以上 → **記錄到 skills 的 Pitfalls 區塊**，防止再犯
4. 子代理報告中的修正建議必須**逐條確認已處理**，不可跳過

### 管線速查

```
選軌道 → python-docx 生成 → LibreOffice 轉 PDF
    → 並行派發 QA 子代理 A（排版）+ B（引用）
    → 兩份報告回來
    → 若全部 ✅ → 交付
    → 若有 ❌ → 修正 → 重新生成 → 重新 QA
```
