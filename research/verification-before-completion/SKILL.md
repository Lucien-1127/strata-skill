...
验证发现错误後，必須立即將学到的教训写回技能，否则下次还会犯同样的错。

```
验证发现错误
  → 判断：这个错误我之前犯过吗？
    → 是：为什么没拦住？SKILL.md 缺了什么？
        → 补上 Pitfall / Prerequisites / 检查点
        → 修改完成後才算「修好」
    → 否：是新错误？
        → 评估：会不会再犯？
        → 会 → 加到相关技能的 Pitfalls 或 Verification
        → 不会 → 记入 session history 即可
  → 继续修复实际 bug
```

**铁则：验证发现错误 = 技能必须更新。**

没有更新技能就报修好的，相当于只修了症状没治病根。
...
**验证后若发现已知模式错误，先 patch skill 再报完成。**
