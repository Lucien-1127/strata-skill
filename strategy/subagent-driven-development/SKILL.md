...
digraph process {
    ...
    "规格审查子智能体确认代码匹配规格?" -> "逻辑验证：真实数据流校验" [label="是"];
    "逻辑验证：真实数据流校验" -> "代码实际执行/API实际呼叫是否返回预期结果?";
    "代码实际执行/API实际呼叫是否返回预期结果?" -> "实现子智能体修复逻辑错误" [label="否"];
    "实现子智能体修复逻辑错误" -> "分派规格审查子智能体 (./spec-reviewer-prompt.md)" [label="重新审查"];
    "代码实际执行/API实际呼叫是否返回预期结果?" -> "分派代码质量审查子智能体 (./code-quality-reviewer-prompt.md)" [label="是"];
...
**质量关卡：**
- 自审在交接前发现问题
- 三阶段审查：规格合规性 → **逻辑正确性（真实数据流校验）** → 代码质量
- 逻辑校验步骤：`curl` 实际 API → 检查回應格式 → 確認資料欄位匹配型別
- 审查循环确保修复确实有效
- 规格合规防止过度/不足构建
- 逻辑正确性确保代码真的能跑、API 真的能回正确数据
- 代码质量确保实现良好
...
- **tsc 编译通过就当「邏輯正確」** — 型別正確 ≠ 資料正確，必須 curl 驗證真實 API 回應
- **子代理說「done」就相信** — 必須獨立驗證：curl 打真實端點、檢查回應格式
